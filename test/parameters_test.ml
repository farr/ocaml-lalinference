...
let test_spinning_logl () = 
  let p = Spinning({m1 = 5.0; m2 = 4.0; dist = 50.0; cos_i = -0.25;
                    psi = 2.6; phi = 5.8; time = 0.0; ra = 2.9;
                    sin_dec = 0.2}, 
                   {a1 = 0.8; a2 = 0.9; cos_tilt1 = 0.2; cos_tilt2 = -0.35; phi1 = 3.1; phi2 = 4.6}) in
    trigtime := 968654558.0;
    psdstart := 968654560.0;
    cache := ["LALLIGO"; "LALLIGO"; "LALVirgo"];
    ifos := ["H1"; "L1"; "V1"];
...
let tests = "parameters.ml tests" >:::
  ["log(L) with known parameters test" >:: test_basic_logl;
   "spinning log(L) with known parameters" >:: test_spinning_logl;
